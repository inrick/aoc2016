...
let neighs (x,y) = [x+1,y;x,y+1;x-1,y;x,y-1]
  |> List.filter ~f:(fun (x,y) -> is_open (x,y) && x >= 0 && y >= 0)
  ...
  let rec go n heap =
    ...
    | Some (pos, heap) ->
      ...
      if dist < steps then
        let ns = neighs pos |> List.filter ~f:(fun c -> not (visited c)) in
        add_neighs (dist+1) ns;
        go (n+1) (List.fold ns ~init:heap ~f:Fheap.add)
      else go n heap in
  go 0 Fheap.(add (create ~cmp:cmp_coords) from)

let () =
  let target = 31, 39 in
  find_path target |> printf "%d\n";
  can_reach (0,0) 50 |> printf "%d\n";
